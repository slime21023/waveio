...
- Client：`WaveClient`、`ClientRequest`、`ClientResponse`、`ClientRequestPool`、`RetryPolicy`、`BackoffPolicy`、redirect/proxy policy，以及內部 `runtime.client.ClientTransport`／`NettyHttp1ClientTransport`。
...
**前置依賴：** 0.8。  
**狀態：** 進行中（0.9-R0–R11 架構整理已完成；0.9-T2/T3 的 RC release evidence 仍待完成）。
...
| 0.9-R2 | `SseResponseInfo`、`ApplicationResult`、`RequestDispatcher`、`BlockingResultWaiter` | SSE open info 與 runtime names 不含舊名；waiter 位於 unexported `runtime.client` | SSE header/line/event/reconnect caps；existing dispatcher lifecycle budgets | 0.9-R0 | success：取得 SSE opening info；application failure：error response/route result propagates；deadline/cancellation：open/idle/cancel checks；limit：header/line/event/reconnect；shutdown：SSE client/emitter/dispatcher stop | 完成 |
| 0.9-R3 | unexported `netty` client/WebSocket transport types | `WaveClient`/`WebSocketClient` public signatures 不引用 Netty implementation；移除單一實作 wrapper/config type | existing connection、stream、header、body、queue、shutdown caps | 0.9-R0、0.9-R1 | success：HTTP/1.1、HTTP/2、WebSocket；application failure：ALPN/handshake/protocol error；deadline/cancellation：connect/disconnect/RST/FIN；limit：admission/header/body/stream；shutdown：physical close、lease release、PARANOID leak | 完成 |
| 0.9-R4 | abstract `Response`、internal `InternalResponse`、`ResponseData`、`ResponseDataReader` | `Response` 只保留 status、headers、writers、lifecycle；移除 `Response.create()`、`Response.body()` 與 public `ResponseBody` | aggregate body、pending response、outbound byte budgets 沿用既有上限 | 0.9-R0、0.9-R3 | success：text/bytes/JSON/problem/stream/WebSocket；application failure：serialization/invalid upgrade；deadline/cancellation：stream cancel/disconnect；limit：aggregate/pending/outbound bytes；shutdown：stream/upgrade/abort | 完成 |
...
| 0.9-R7 | `api.application.WaveApp`、`api.server.WaveServer`／`RunningServer`、`api.http` 的 `BodyPublisher`／`Http2Config`／`PublicAddress` | 根 package 只保留 `Wave`；移除 `api.stream` export；`Response` 不引用 internal implementation；`ResponseUpgrade` 是 HTTP-to-upgrade 的唯一窄介面 | static package boundary only；沿用既有 response/body budgets | 0.9-R0–R6 | success：five named-module examples compile；application failure：舊 package/import fails; deadline/cancellation：不適用；limit：public exports fixed；shutdown：不適用 | 完成 |
| 0.9-R8 | `BuiltInWaveFactory`、`runtime.server.ServerTransport`、`runtime.client.ClientTransport`、`DefaultWaveServer`、`DefaultWaveClient` | `Wave` 是唯一 built-in assembly entry；runtime does not depend on Netty；Netty does not depend on root `Wave` | existing server/client connection, request, header, body, queue and shutdown budgets | 0.9-R7 | success：HTTP/1.1 and HTTP/2 clients/servers retain contracts；application failure：transport failure propagates; deadline/cancellation：connect, disconnect, deadline; limit：admission and byte caps; shutdown：server/client close | 完成 |
| 0.9-R9 | `NettyServerTransport`、`NettyHttp1ClientTransport`、`NettyHttp2ClientTransport`、`NettyWebSocketClient` | Bootstrap/EventLoopGroup/ChannelInitializer/pipeline/ByteBuf/channel lifecycle remain in `netty.*`; public clients are operation contracts created by `Wave` with `*Options` | existing connection, stream, header, body, queue and leak budgets | 0.9-R8 | success：HTTP/1.1, HTTP/2, WebSocket real socket tests；application failure：ALPN/handshake/protocol errors; deadline/cancellation：connect/RST/FIN; limit：admission/header/body/stream; shutdown：physical close and PARANOID leak | 完成 |
| 0.9-R10 | `NettySseClient`、`SseClientOptions`、incremental SSE parser | SSE stays direct HTTP/1.1; ByteBuf is copied/released inside the handler; callbacks run on virtual threads and manual reads apply TCP backpressure | four pending transport signals, maximum connections, header/line/event/retry/idle budgets | 0.9-R8 | success：chunked/BOM/event/reconnect; application failure：invalid header/line/event fails; deadline/cancellation：slowloris, idle, close; limit：header/line/event/admission; shutdown：permit/event-loop cleanup and PARANOID leak | 完成 |
| 0.9-R11 | ArchUnit, JPMS manifest, reflection boundary tests, naming/roadmap validators, migration docs | API slices acyclic; root package only exposes `Wave`; no API/SPI implementation dependency; no public Netty/JDK HTTP/socket signature | static validation only；不新增 runtime resource | 0.9-R7–R10 | success：validators and complete verify pass；application failure：export/name/dependency drift fails; deadline/cancellation：不適用；limit：public surface and task schema fixed；shutdown：不適用 | 完成：2026-09-10 validators、verify、transport 與 reliability gates 均通過 |
| 0.9-R12 | JaCoCo coverage profile、layered coverage validator、contract/transport coverage suites | bundle and public/runtime/netty/internal layers require line ≥ 90%、branch ≥ 80%；reports remain build artifacts only | one instrumented normal verify; transport remains a separate PARANOID leak run; no new runtime resource | 0.9-R11 | success：report and all layer thresholds pass；application failure：missing/invalid report or a threshold breach fails closed；deadline/cancellation：targeted client/server/stream cancellation tests；limit：admission/header/body/queue tests；shutdown：bounded cleanup tests | 進行中 |
...
0.9-R0–R11 收尾證據：`pwsh ./tools/roadmap/validate-roadmap.ps1`（41 筆任務）與
`pwsh ./tools/roadmap/validate-naming.ps1`（212 production files）均通過；
`./mvnw.cmd -B -ntp verify` 與 `./mvnw.cmd -B -ntp -pl wave -am verify -Ptransport` 各通過
331 tests，`./mvnw.cmd -B -ntp -pl wave -am verify -Ptransport,reliability` 通過 332 tests，均為
0 failures/errors/skips。`Response` abstract/public boundary、`api.testing` 移除、Netty
client/WebSocket implementation 移至未 export package，以及五個 named-module examples 均以目前
source tree 驗證。這裡停止於 1.0 API stability 之前，不建立
