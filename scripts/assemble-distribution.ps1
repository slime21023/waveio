...
$modules = @("foundation", "execution", "task", "http", "netty", "server", "testkit")
foreach ($module in $modules) {
    $source = Join-Path $root "waveio-$module/target"
    Get-ChildItem -LiteralPath $source -Filter "waveio-$module-$Version*.jar" | Copy-Item -Destination (Join-Path $output "artifacts")
    Copy-Item -LiteralPath (Join-Path $source "dependency-report.txt") -Destination (Join-Path $output "reports/waveio-$module-dependencies.txt")
}
